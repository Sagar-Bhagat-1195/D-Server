...
// const Super_Admin = require("./super_admin.model");
...
exports.SignUp = async function (req, res, next) {
  try {
    ...
    let existing_Admin = await Super_Admin.findOne({ email: req.body.email });
    ...
    const newAdmin = await Super_Admin.create(req.body);
...
exports.Login = async function (req, res, next) {
  try {
    ...
    const checkUser = await Super_Admin.findOne({ email: req.body.email });
...
exports.Update = async function (req, res, next) {
  try {
    console.log("Super Admin Update :)")
    // if(!req.body || !req.body.name || !req.body.email || !req.body.password){
    //     throw new Error("Please enter valid fields")
    // }
    ...
    // let Update_Admin = await Super_Admin.findByIdAndUpdate(req.adminId, req.body, {
    //   new: true,
    // });
    let old_Admin_Data = {};
    let Admin = await Super_Admin.findById(req.Super_AdminId);
...
exports.AllAdmin = async function (req, res, next) {
  try {
    let allAdmin = [];
    if (req.params.id) {
      allAdmin = await Super_Admin.findById(req.params.id);
    } else {
      allAdmin = await Super_Admin.find().select("name phone");
    }
    res.status(201).json({
      message: "Success",
      data: allAdmin,
...
exports.DeleteAdmin = async function (req, res, next) {
  try {
    ...
    let allAdmin = await Super_Admin.findByIdAndDelete(req.params.id);
